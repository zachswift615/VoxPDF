...
fn spike_validates_word_positions() {
    ...
    assert!(
        ...
        "Hello X position: expected ~100, got {}",
        hello.bounds.x
    );
    assert!(
    ...
    assert!(
        (world.bounds.x - 160.0).abs() < 16.0,
        "World X position: expected ~160, got {}",
        world.bounds.x
    );
    assert!(
    ...
    assert!(
        hello.bounds.width > 0.0 && hello.bounds.width < 100.0,
        "Hello width: expected >0 and <100, got {}",
        hello.bounds.width
    );
    assert!(
        hello.bounds.height > 0.0 && hello.bounds.height < 30.0,
        "Hello height: expected >0 and <30, got {}",
        hello.bounds.height
    );
}
